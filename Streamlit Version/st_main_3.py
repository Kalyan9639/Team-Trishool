...
def analyze_resumes(job_title, job_description, required_skills, groq_api_key, files):
...
def get_verdict_class(verdict):
    ...
    if any(word in verdict_lower for word in ['excellent', 'outstanding', 'perfect']):
        return 'verdict-excellent'
    elif any(word in verdict_lower for word in ['good', 'suitable', 'qualified']):
        return 'verdict-good'
    else:
        return 'verdict-poor'
...
with st.sidebar:
    ...
    groq_api_key = st.text_input(
        ...
        help="Required for AI verdict & feedback generation"
    )
    ...
    if st.button("🚀 Start Analysis", use_container_width=True, disabled=st.session_state.processing):
        if not all([job_title, job_description, required_skills]):
            st.error("Please fill all job-related fields")
...
if st.session_state.analysis_results:
    ...
    # Display candidates
    for idx, candidate in enumerate(results['top_candidates']):
