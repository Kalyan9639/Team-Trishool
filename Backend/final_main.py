...
from langchain_groq import ChatGroq
from langchain_core.prompts import ChatPromptTemplate
# --- REMOVED: JsonOutputParser is no longer needed ---
...
app = FastAPI(
    title="Intelligent Resume Ranker API V4 (Cloud Ready - Fixed)",
    description="A LangGraph-powered resume screener using Structured Output for reliable AI feedback.",
    version="4.1.0"
)

# --- MODEL LOADING (UNCHANGED) ---
# This loads the model once when the application starts.
logger.info("Loading SentenceTransformer model 'all-mpnet-base-v2' into memory...")
embedding_model_instance = SentenceTransformer("all-mpnet-base-v2")
...
# --- NEW: Pydantic Model for Structured AI Output ---
# This class defines the exact structure we want the LLM to return.
# This replaces the need to parse unpredictable JSON strings.
class AIVerdict(BaseModel):
    """A formal HR analysis of a candidate's resume."""
    ai_verdict: str = Field(description='A short, direct verdict like "Strong Fit", "Potential Fit", or "Lacks Key Skills".')
    ai_feedback: str = Field(description="A 2-3 sentence formal analysis explaining the verdict based on the evidence.")
...
class ResumeWorkflow:
    def __init__(self):
        ...
        # Pass the pre-loaded model to the LangChain wrapper
        self.embedding_model = SentenceTransformerEmbeddings(model_name=None, model=embedding_model_instance)
        logger.info("✅ Embeddings wrapper initialized.")

    # --- preprocess_resumes, semantic_search, skill_rerank are UNCHANGED ---
    def preprocess_resumes(self, state: AgentState) -> AgentState:
        logger.info("--- Starting Node: Preprocess Resumes ---")
        processed_resumes = {}
        section_documents = []
    ...
    # --- MODIFIED: This function is now robust and reliable ---
    async def generate_feedback(self, state: AgentState) -> AgentState:
        logger.info("--- Starting Node: Generate AI Feedback (Structured Output) ---")
        if not state["groq_api_key"]:
            logger.warning("No Groq API key provided. Skipping AI feedback generation.")
            return state
        ...
        try:
            ...
            structured_llm = llm.with_structured_output(AIVerdict)
            
            prompt = ChatPromptTemplate.from_messages([
                ("system", "You are a formal HR analyst. Your task is to provide a direct, concise analysis of a candidate's fit for a role based on their resume sections and the job's required skills."),
                ("human", """Analyze the candidate for a role requiring: {required_skills}.
                **Candidate's Experience:** {experience_section}
                **Candidate's Skills:** {skills_section}"""),
            ])
            
            # The chain now reliably outputs AIVerdict objects
            chain = prompt | structured_llm
        except Exception as e:
            logger.error(f"Failed to initialize Groq model with structured output: {e}. Skipping feedback.")
            return state
        ...
        async def get_feedback_for_candidate(candidate):
            ...
            try:
                # The output is a Pydantic object, not a string. No parsing needed!
                response_object = await chain.ainvoke({
                    "required_skills": ", ".join(state["required_skills"]),
                    "experience_section": sections.get('experience', 'Not specified.'),
                    "skills_section": sections.get('skills', 'Not specified.')
                })
                # Convert the object to a dictionary and update the candidate data
                candidate.update(response_object.dict())
            except Exception as e:
                logger.error(f"Error generating structured feedback for {filename}: {e}")
                candidate["ai_verdict"] = "Generation Error"
                candidate["ai_feedback"] = "Could not generate structured AI feedback."
            return candidate
